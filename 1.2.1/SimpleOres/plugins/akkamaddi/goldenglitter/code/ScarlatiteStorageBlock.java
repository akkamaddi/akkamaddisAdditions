...
public class ScarlatiteStorageBlock extends Block
{
    ...
    public ScarlatiteStorageBlock(int par1, Material material, String mod)
    {
        super(par1, material);
        this.modName = mod;
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random)
    {
        if (GoldenGlitterCore.MakeScarlatiteSparkle == true)
        {
            ...
            world.spawnParticle("instantSpell", (double)((f1 + 0.3) + (f6 * 0.7)), (double)((f2 + 0.3) + (f4 * 0.7)) , (double)((f3 + 0.3) + (f5 * 0.7)), 0.0D, 0.0D, 0.0D);
        }
        else
        {
            return;
